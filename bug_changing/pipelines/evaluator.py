...
class Evaluator:
    ...
    def evaluate(self, summary_pair, instance_summary_pairs, all_targets, project, with_extractor_instances, with_discriminator_instances):
        answer, raw_answer = self.extractor.extract_info_from_summary_pair(summary_pair, instance_summary_pairs,
                                                                           all_targets, project,
                                                                           with_extractor_instances)
        answer, raw_answer = self.discriminator.discriminate_info_from_ans(summary_pair, answer, raw_answer,
                                                                           instance_summary_pairs, project,
                                                                           with_discriminator_instances)
...
class Extractor:
    ...
    def get_instances(self, summary_pairs, project=MOZILLA_PROJ):
        ...
        if project == MOZILLA_PROJ:
            bug_id_summary_ids_list = [(1115363, 1, 2), (1213421, 0, 1), (1300525, 0, 1),
                                       (1122695, 0, 1), (1403460, 0, 1), (677017, 0, 1), (1725890, 0, 1)]
        else:
    ...
    def convert_instances_into_qa_pairs(self, all_targets=None):
        qa_pairs = []
        if self.instances:
            for instance in self.instances:
                self.question_for_summary(instance.rm_summary, all_targets)
    ...
    def get_session_prompt(self, all_targets=None):
        # self.session_prompt = f"I am a text extractor. " \
        #                       "I can extract specific parts directly from a given bug report. " \
        #                       "All extracted parts are the substrings of the bug report. " \
        ...
        if all_targets:
            ...
            for target in self.targets:
                target_info = f"\t{target.name}: {target.explanation}\n"
                self.session_prompt = self.session_prompt + target_info
        else:
            self.session_prompt = f"I am a text extractor. " \
                                  "I can extract specific parts directly from a given bug report. " \
                                  "All extracted parts are the substrings of the bug report. " \
        ...
        # target_intro = f"{Placeholder.SUGGESTED_SOLUTION}: the proposed fix for the issue\n" \
    ...
    def question_for_summary(self, summary, all_targets=None):
        # self.session_prompt = self.session_prompt + target_intro
        target_info = ''
        if all_targets:
            target_info = 'The specific parts are as follows:\n'
            self.get_targets(all_targets)
            for target in self.targets:
                target_info = target_info + f"\t{target.name}: {target.explanation}\n"
        self.summary_question = f"Bug summary: {summary.text}\n" \
                                f"Please only extract do not infer the {Placeholder.SUGGESTED_SOLUTION}, " \
                                ...
                                # f"All extracted parts are the substrings of the bug summary and none of them is from the inference. " \
                                # f"If don't exist, respond with None):\n"
        # if target_info:
        self.summary_question = target_info + self.summary_question
        # self.summary_question = f"Bug summary: {summary.text}\n" \
    ...
    def get_initial_messages(self, instance_summary_pairs, all_targets, project=MOZILLA_PROJ, with_instances=True):
        """
        Args:
            all_targets ():
            instance_summary_pairs ():
            project ():
            with_instances ():
        ...
        """
        self.get_session_prompt(all_targets)
        qa_pairs = None
        if with_instances:
            self.get_instances(instance_summary_pairs, project)
    ...
    def extract_info_from_summary_pair(self, summary_pair, instance_summary_pairs, all_targets, project, with_instances=True):
        messages = self.get_initial_messages(instance_summary_pairs, all_targets, project, with_instances)
        # extract summary
        self.question_for_summary(summary_pair.rm_summary)
        messages = LLMUtil.add_role_content_dict_into_messages(LLMUtil.ROLE_USER, self.summary_question, messages)
        # print(self.summary_question)
        # input()
        summary_answer = LLMUtil.ask_turbo(messages)
        messages = LLMUtil.add_role_content_dict_into_messages(LLMUtil.ROLE_ASSISTANT, summary_answer, messages)
        # summary_answer = Answer.extract_answers(summary_answer)
        # extract description
        self.question_for_description(summary_pair.bug.description)
        messages = LLMUtil.add_role_content_dict_into_messages(LLMUtil.ROLE_USER, self.desc_question, messages)
        # print(self.summary_question)
        # input()
        desc_answer = LLMUtil.ask_turbo(messages)
        messages = LLMUtil.add_role_content_dict_into_messages(LLMUtil.ROLE_ASSISTANT, desc_answer, messages)
    ...
    def extract_info_from_desc(self, description, instance_summary_pairs, all_targets, project, with_instances):
        """
        ...
        """
        # self.get_session_prompt(all_targets)
        # messages = LLMUtil.get_messages_for_turbo(self.session_prompt)
        messages = self.get_initial_messages(instance_summary_pairs, all_targets, project, with_instances)
        # extract description
...
class Discriminator:
    ...
    def get_instances(self, summary_pairs, project=MOZILLA_PROJ):
        """
        Mozilla: 9 instances -> 5 instances
            ...
            (1583398, 0, 1): is_actual_behavior_vague_vague(specific); present tense(did)
                # https://bugzilla.mozilla.org/show_bug.cgi?id=1122695
                ...
                # (1639575, 0, 1): syntax (don't -> doesn't)
            https://bugzilla.mozilla.org/show_bug.cgi?id=1693854
            (1693854, 0, 1): typos (doen't)

        Eclipse: 12 instances -> 5 instances
                is_actual_behavior_vague(4); is_trigger_action_vague();
                 spelling error(0); syntax error(1);
                 declarative statement(); present tense(); personal pronoun(); emotional reaction();
                 offensive words(); wishy-washy words();
            https://bugs.eclipse.org/bugs/show_bug.cgi?id=539445
            (539445, 0, 1) is_actual_behavior_vague(add details);
            https://bugs.eclipse.org/bugs/show_bug.cgi?id=328766
            (328766, 0, 1) is_actual_behavior_vague(from vague to specific);
            https://bugs.eclipse.org/bugs/show_bug.cgi?id=468457
            (468457, 0, 1) is_actual_behavior_vague(add conditions);
            https://bugs.eclipse.org/bugs/show_bug.cgi?id=319680
            (319680, 0, 1) is_actual_behavior_vague(correction);
            # https://bugs.eclipse.org/bugs/show_bug.cgi?id=312867
            # (312867, 0, 1) is_actual_behavior_vague(from vague to specific);
            # https://bugs.eclipse.org/bugs/show_bug.cgi?id=387529
            # (387529, 0, 1) spelling error
            https://bugs.eclipse.org/bugs/show_bug.cgi?id=435156
            (435156, 0, 1) syntax error
        Args:
            summary_pairs ():
            project ():
        ...
        if project == MOZILLA_PROJ:
            ...
            bug_id_summary_ids_list = [(539445, 0, 1),
                                       # (569698, 0, 1),
                                       (328766, 0, 1),
                                       # (404912, 0, 1),
                                       (468457, 0, 1), (319680, 0, 1),
                                       # (312867, 0, 1),
                                       # (345467, 0, 1),
                                       # (353615, 0, 1),
                                       # (387529, 0, 1),
    ...
    def question_for_actual_behavior(self, summary_actual_behavior, description_actual_behavior):
        ...
        if summary_actual_behavior is not None and description_actual_behavior is not None:
            self.actual_behavior_question = f"Can the actual behavior from the bug summary {Placeholder.TAG_ACTUAL_BEHAVIOR[0]}{summary_actual_behavior}{Placeholder.TAG_ACTUAL_BEHAVIOR[1]} be refined based on the actual behavior from the bug description " \
    ...
    def question_for_trigger_action(self, summary_trigger_action, description_trigger_action):
        ...
        if summary_trigger_action is not None and description_trigger_action is not None:
            self.trigger_action_question = f"Can the trigger action from the bug summary {Placeholder.TAG_TRIGGER_ACTION[0]}{summary_trigger_action}{Placeholder.TAG_TRIGGER_ACTION[1]} be refined based on the trigger action from the bug description " \
    ...
    def get_initial_messages(self, instance_summary_pairs, project=MOZILLA_PROJ, with_instances=True):
        """
        Args:
            instance_summary_pairs ():
            project ():
    ...
    def discriminate_info_from_ans(self, summary_pair, answer, raw_answer, instance_summary_pairs, project, with_instances=True):
        messages = self.get_initial_messages(instance_summary_pairs, project, with_instances)
        ...
        if answer is not None and raw_answer is not None:
            ...
            LLMUtil.show_messages(messages)
            raw_answer.discriminator = QA(self.vague_understandable_concise_question, ans)
