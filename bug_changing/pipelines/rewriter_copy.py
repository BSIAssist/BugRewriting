...
class Rewriter:
    def __init__(self):
        ...
        self.session_prompt = "I'm a bug summary rewriter. " \
                              "I can rewrite the bug summary based on the bug description. " \
                              "The rewritten bug summary is around 13 words and satisfies the given conditions.\n"
    ...
    # def get_session_prompt(self, answer, goal_model, project):
    #     """
    ...
    #         session_prompt = session_prompt + f"KG: Bug summary: {instance.rm_summary.text}\n"
    #         session_prompt = session_prompt + f"    Bug description: <DESCRIPTION>{instance.bug.description.text}</DESCRIPTION>\n"
    #         session_prompt = session_prompt + self.rewrite_prompt
    #         prompt = instance.hard_goals.generate_prompt()
    #         session_prompt = session_prompt + prompt
    #         session_prompt = session_prompt + self.rewritten_prompt
    #         session_prompt = session_prompt + f"\nAI: {instance.add_summary.text}\n"
    ...
    def get_instances_by_hard_goal_repeat_num(self, hard_goal_repeat_num=2, project=MOZILLA_PROJ):
        ...
        if self.hard_goals:
            ...
            instance_hd_intersection_num_hd_diff_num_tuples = Rewriter.get_instance_intersection_diff_hardgoals_num_tuple_list(hard_goals, project)
            if instance_hd_intersection_num_hd_diff_num_tuples:
                index = 0
                while True:
                    if instance_hd_intersection_num_hd_diff_num_tuples:
    ...
    def convert_instances_into_prompt(self, rewrite_question_with_extrator_result=True):
        ...
        if self.instances:
            for instance in self.instances:
                ...
                instance_prompt = instance_prompt + self.question_from_evaluated_answer(instance, instance.rm_summary, False,
    ...
    def rewrite(self, summary_pair, answer, raw_answer, goal_model, project, with_instances=True,
                rewrite_question_with_extrator_result=True, llm=None):
        """
        ...
        if answer is not None and raw_answer is not None:
            ...
            if llm is None:
                ...
                if LLMUtil.reach_chatgpt_limited_request(ans, summary_pair):
                    return None, None
                # question = self.question_for_evaluated_answer
            raw_answer.rewriter = QA(question, ans)
            ans = ans.replace('\n', ' ').replace('\r', '')
