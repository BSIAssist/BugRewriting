...
from bug_changing.types.product_component_pair import ProductComponentPairFramework
from bug_changing.types.summary import SummaryPairs
from bug_changing.types.tossing_path import TossingPathFramework
from bug_changing.utils.graph_util import Edge, MyNode
from bug_changing.utils.nlp_util import NLPUtil
from config import WORD2VEC_DIM, ONEHOT_DIM, MOZILLA_PROJ
...
class Bugs:
    ...
    def random_sample_bugs(self, sample_num=384):
        sample_bugs = []
        sample_num_list = random.sample(range(0, self.get_length()), sample_num)
        for sample_num in sample_num_list:
            sample_bugs.append(self.bugs[sample_num])
        return Bugs(sample_bugs)

    def get_bug_by_id(self, bug_id):
        for bug in self.bugs:
            if bug.id == int(bug_id):
                return bug

    def get_bug_by_summary(self, summary):
        for bug in self.bugs:
            if bug.summary == summary:
                return bug

    def get_bug_by_input(self):
        """
        get bug by the bug id and its bug_summary by the summary id
        Returns: bug, bug_summary
        """
        print("Please input the bug id:")
        bug_id = input()
        bug = self.get_bug_by_id(bug_id)
        # from 0, 1, ...
        print("Please input its summary id:")
        summary_id = input()
        # bug_summary = bug.summary_path[int(summary_id)]
        return bug, int(summary_id)
    ...
    def filter_bugs_by_desc_tokens_num(self, min_tokens_num=10, max_tokens_num=300):
        bug_list = []
        for bug in tqdm(self.bugs, ascii=True):
            if bug.description:
                token_num = NLPUtil.calculate_token_num(bug.description.text)
                if min_tokens_num <= token_num <= max_tokens_num:
                    bug_list.append(bug)
        return Bugs(bug_list)

    def filter_bugs_by_desc_with_log(self, folder_name=MOZILLA_PROJ):
        """
        ...
        filtered_bugs = []
        for bug in tqdm(self.bugs, ascii=True):
            if '```' in bug.description.text:
                description = bug.description.text.lower()
                if 'log:' in description or 'failure log' in description or 'crash report' in description \
                        or 'stack trace' in description or 'assertion failure' in description:
                    # print(bug.summary)
    ...
    def get_summary_reformulation_bugs(self):
        """
        get bugs with summary reformulation
        """
        summary_reformulation_bugs = []
        for bug in tqdm(self.bugs, ascii=True):
            if len(bug.summary_path) > 1:
                summary_reformulation_bugs.append(bug)
        return Bugs(summary_reformulation_bugs)

    def get_goal_oriented_summary_pairs(self):
        """

        Returns: SummaryPairs(summary_pairs)

        """
        summary_pairs = []
        for bug in self.bugs:
            summary_pairs.extend(bug.get_goal_oriented_summary_pairs())
            # input()
    ...
    def get_summary_list(self):
        ...
        summary_list = []
        for bug in self.bugs:
            summary_list.append(bug.summary)
        return summary_list

    def get_ith_summary_list(self, ith):
        """
        get bugs' summary
        :return: bug summary list
        """
        summary_list = []
        for bug in self.bugs:
            summary_list.append(bug.summary_path[ith].text)
        return summary_list

    def get_rollback_summary_list(self):
        """
        get bugs' summary
        :return: bug summary list
        """
        rollback_summary_list = []
        for bug in self.bugs:
            rollback_summary_list.append(bug.summary_path[0])
        return rollback_summary_list
    ...
    def get_description_list(self):
        ...
        description_list = []
        for bug in self.bugs:
            description_list.append(bug.description)
        return description_list

    def get_summary_description_list(self):
        """
        get bugs' summary. description
        :return: bug summary. description list
        """
        summary_description_list = []
        for bug in tqdm(self.bugs, ascii=True):
            summary_description_list.append(f"{bug.summary}. {bug.description}")
        return summary_description_list

    def get_pc_index_labels(self, product_components):
        """
        get p::c labels (int) for bugs
        Args:
            product_components ():
    ...
    def split_dataset_by_radio(self, radio=0.8):
        """
    ...
    def split_dataset_by_creation_time(self, creation_time="2021-01-01T00:00:00Z"):
        """
        sort bugs by creation time
        :return:
        """
    ...
    def split_dataset_by_summary_rewritten_num(self, rewritten_num=2):
        """
        sort bugs by summary_rewritten_num
        A -> B -> C: rewritten num = 2
        :return:
        """
        train_bugs = list()
        test_bugs = list()
        # i = 0
        for bug in self.bugs:
            if len(bug.summary_path) - 1 < rewritten_num:
                train_bugs.append(bug)
            else:
                test_bugs.append(bug)
            # i = i + 1
        train_bugs = Bugs(train_bugs)  # summary rewritten once
        # train_bugs.overall_bugs()
        test_bugs = Bugs(test_bugs)  # summary rewritten >=2
        # test_bugs.overall_bugs()
        return train_bugs, test_bugs
    ...
    def get_desc_token_num_distribution(self):
        token_num_count_dict = {}
        for bug in tqdm(self.bugs, ascii=True):
            if bug.description:
                token_num = NLPUtil.calculate_token_num(bug.description.text)
            else:
                token_num = 0
            token_num_count_dict[token_num] = token_num_count_dict.get(token_num, 0) + 1
        return token_num_count_dict
